...
const ShopNowButton = () => {
  ...
  const products = [
    { id: 1, name: "Nutrimix  500g", price: 400 },
    { id: 2, name: "Nutrimix  200g", price: 160 },
    { id: 3, name: "Nutrimix  100g", price: 100 },
  ];

  const handleProductChange = (
    e: React.ChangeEvent<HTMLInputElement>,
    product: { id: any; name?: string; price?: number }
  ) => {
    ...
    if (checked) {
      ...
      setSelectedProducts(
        selectedProducts.filter((item: any) => item.id !== product.id)
      );
      const updatedQuantities = { ...selectedQuantities };
      delete updatedQuantities[product.id];
      setSelectedQuantities(updatedQuantities);
  ...
  const calculateTotal = () => {
    return selectedProducts.reduce(
      (total: number, product: { id: number; price: number }) => {
  ...
  const handleSendWhatsApp = () => {
    const { name, email, street, city, state, postalCode, country, phone } =
      formData;
    if (
      !name ||
      !street ||
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
          <div className="relative bg-white w-full max-w-lg p-8 rounded-3xl shadow-lg">
            <div className="flex justify-between mb-6">
              {currentTab === "products" ? (
                <div
                  onClick={() => setCurrentTab("products")}
                  className="mt-1 w-full p-1 font-bold  text-[#bc8a27] rounded-full border-b-2"
                >
                  Select Products
                </div>
              ) : (
                  ...
                  {products.map((product) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex ">
                          {/* <img
      src={product?.image || "/default-product.jpg"} // Replace with your product image URL or a default placeholder
      alt={product.name}
    ...
    /> */}
                      ...
                      {selectedProducts.find(
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <input
                            ...
                            value={selectedQuantities[product.id] || 1}
                            onChange={(e) => handleQuantityChange(e, product)}
                            className="w-16 p-2 text-center border border-gray-300 rounded-lg focus:ring-green-400"
                            min="1"
                          />
                          <button
                            onClick={() =>
            ...
            {currentTab != "products" && (
              <button
                onClick={handleSendWhatsApp}
                className="mt-6 w-full py-3 bg-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 transition-transform transform hover:scale-105"
              >
                Send Order via WhatsApp
              </button>
            )}
            ...
            <button
                onClick={() => setIsModalOpen(false)}
                className="mt-4 mr-2 ml-2 w-full py-3 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-600 transition-transform transform hover:scale-105"
              >
                Close
              </button>
              {currentTab === "products" ? (
                <button
                  onClick={() => setCurrentTab("address")}
                  className="mt-4 mr-3  w-full py-3 bg-yellow-500 text-white font-semibold rounded-lg shadow-md hover:bg-yellow-600 transition-transform transform hover:scale-105"
                >
                  Continue
                </button>
              ) : (
                <button
                  onClick={() => setCurrentTab("products")}
                  className="mt-4 w-full py-3 bg-yellow-500 text-white font-semibold rounded-lg shadow-md hover:bg-yellow-600 transition-transform transform hover:scale-105"
                >
                  Back
                </button>
              )}
