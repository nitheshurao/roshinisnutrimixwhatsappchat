...
const ReviewSection = ({ productId }:any) => {
    ...
    async (pageNumber = 1) => {
      ...
      try {
        ...
        if (pageNumber === 1) {
          ...
          setReviews((prevReviews: any) => [...prevReviews, ...res.data.reviews]);
        }
  ...
  const handleReviewSubmit = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    console.log("--handleReviewSubmit",userReview,session)
    e.preventDefault();
    if (!session) {
      router.push("/login")
    }
    try {
      await axios.post("/api/review", {userId: session?.user?._id,productId, ...userReview });
  ...
  const handleUpdateReview = async (e: { preventDefault: () => void; }) => {
    e.preventDefault();
  ...
  return (
    <div className="mt-12 bg-gradient-to-b from-gray-50 to-white rounded-xl shadow-lg p-4 md:p-8">
      ...
      />
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="px-4 py-2 bg-[#cd7b4b] text-white rounded-lg hover:bg-[#867558] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200"
              >

               { !session? "please login ":editingReview ? "Update Review" : "Submit Review "}
      ...
      <div className="space-y-4 md:space-y-6">
        {reviews.map((review: { _id?: any; user?: any; createdAt?: any; rating: any; comment: any; }) => (
          <motion.div
            key={review._id}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
